...
usersRouter.get('/all', authenticate, (req: Request, res: Response) => {
    ...
    pool.getConnection(function (err : any, conn : any) {
        ...
        pool.query('SELECT id, email, mobile FROM users', function(err : any, rows : any) {
            if(err) {
                conn.release();
                return res.send({
                    success: false,
...
usersRouter.get('/details/:id', authenticate, (req: Request, res: Response) => {
        ...
        pool.getConnection(function (err : any, conn : any) {
            ...
            // if you got a connection...
            pool.query('SELECT * FROM users WHERE id=?', [req.params.id], function(err : any, rows : any) {
                if(err) {
                    conn.release();
                    return res.send({
                        success: false,
...
usersRouter.post('/login', (req: Request, res: Response) => {
    pool.getConnection(function (err : any, conn : any) {
        ...
        console.log(req.body);
        pool.query('SELECT password FROM users WHERE email=?', [req.body.email], function(err : any, rows : any) {
            if(err) {
                conn.release();
                return res.send({
                    success: false,
